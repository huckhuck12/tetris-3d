...
确保你的环境中已安装 [Node.js](https://nodejs.org/) (建议 v16+)。
...
2.  将 `dist` 文件夹中的内容推送到你的 GitHub 仓库的 `gh-pages` 分支，或者直接将 `dist` 内容作为静态网站托管。
    *   *注意*: `vite.config.ts` 中已配置 `base: './'`，支持相对路径部署，因此你可以将其部署在任何子路径下。
